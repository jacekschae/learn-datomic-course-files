...
(defn transact-step
  ...
  (d/transact conn {:tx-data [{:recipe/recipe-id (parse-uuid recipe-id)
                               :recipe/steps [{:step/step-id (parse-uuid step-id)
...
(defn retract-step
  ...
  (d/transact conn {:tx-data [[:db/retractEntity [:step/step-id (parse-uuid step-id)]]]}))

(defn transact-ingredient
  [{:keys [conn]} {:keys [recipe-id ingredient-id amount measure sort name]}]
  (d/transact conn {:tx-data [{:recipe/recipe-id (parse-uuid recipe-id)
                               :recipe/ingredients [{:ingredient/ingredient-id (parse-uuid ingredient-id)
